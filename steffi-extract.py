#!/usr/bin/env python3
"""
steffi-extract

Decode a base64-encoded ZIP archive and extract its contents to a directory.
Insert the full base64 string into the placeholder variable below and run.
"""

import argparse
import base64
import os
import zipfile
from io import BytesIO


def _decode_base64_to_bytes(base64_text: str) -> bytes:
    """Normalize and decode a base64 string into raw bytes."""
    normalized_b64 = base64_text.strip()
    if normalized_b64.startswith("data:"):
        try:
            normalized_b64 = normalized_b64.split(",", 1)[1]
        except Exception:
            pass
    normalized_b64 = "".join(normalized_b64.split())
    missing_padding = len(normalized_b64) % 4
    if missing_padding:
        normalized_b64 += "=" * (4 - missing_padding)
    return base64.b64decode(normalized_b64)


def main() -> None:
    """Entry point for the steffi-extract script."""
    # Paste the full base64 string between the quotes below (or override via CLI)
    base64_string = "   ..."

    parser = argparse.ArgumentParser(description="Decode base64 ZIP and extract files")
    parser.add_argument("--input-file", help="File containing the base64 data")
    parser.add_argument("--env-var", help="Env var name containing the base64 data")
    parser.add_argument("--output-dir", default="steffi_wakeapp", help="Extraction directory")
    parser.add_argument("--save-zip", help="Path to save decoded bytes (.zip)")
    args = parser.parse_args()

    # Resolve base64 source
    if args.input_file:
        if not os.path.isfile(args.input_file):
            raise SystemExit(f"Input file not found: {args.input_file}")
        with open(args.input_file, "r", encoding="utf-8") as f:
            base64_string = f.read()
    elif args.env_var and os.getenv(args.env_var):
        base64_string = os.getenv(args.env_var, "")

    # Directory to extract the ZIP contents into
    output_directory = args.output_dir

    if not base64_string or base64_string == "HIER_DEN_GESAMTEN_BASE64_STRING_EINFÜGEN":
        raise SystemExit("Provide base64 via --input-file/--env-var or paste into base64_string.")

    os.makedirs(output_directory, exist_ok=True)

    try:
        zip_bytes = _decode_base64_to_bytes(base64_string)
    except Exception as decode_error:  # noqa: BLE001 - broad for user convenience
        raise SystemExit(f"Failed to decode base64 input: {decode_error}")

    if args.save_zip:
        os.makedirs(os.path.dirname(args.save_zip) or ".", exist_ok=True)
        with open(args.save_zip, "wb") as f:
            f.write(zip_bytes)
        print(f"Saved decoded bytes to: {args.save_zip}")

    zip_buffer = BytesIO(zip_bytes)

    try:
        with zipfile.ZipFile(zip_buffer, "r") as zip_ref:
            zip_ref.extractall(output_directory)
    except zipfile.BadZipFile as bad_zip_error:
        msg = (
            "Decoded data is not a valid ZIP file. Use --save-zip to save the raw bytes and verify the input."
        )
        raise SystemExit(f"{msg} Details: {bad_zip_error}")
    except Exception as extract_error:  # noqa: BLE001 - broad for user convenience
        raise SystemExit(f"Failed to extract ZIP contents: {extract_error}")

    print(f"Extraction complete. Files written to '{output_directory}'.")


if __name__ == "__main__":
    main()

